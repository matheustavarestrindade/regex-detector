...
export const initCurrencyModel = async (connection) => {
    CurrencyModel.init(
        {
            id: {
                unique: true,
    ...
    await CurrencyModel.sync();
};

export const prefillCurrencyModel = async () => {
    ...
    for (const key of Object.keys(currencyList)) {
        const name = currencyList[key];
        await CurrencyModel.upsert({
            symbol: key,
            name,
        });
    }
